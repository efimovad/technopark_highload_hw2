...
По данным выше среднее rps = 229,44 млн * 11 / (7 * 24 * 60 * 60) = 4173
...
Так как с развитием сервиса и с увеличением проникновения интернета количество пользователей может увеличиться, а значит и увеличится количество запросов. Также максимальная нагрузка может быть в несколько раз больше средней, необходимо ориентироваться на большее значение нагрузки.
По причинам описанным выше будем ориентироваться на нагрузку в 2 раза большую, чем средняя. Тогда rps_макс = 8346 

### Логическая схема базы данных
Основные запросы к бд:
- получить список объявлений по расположению, по категории, по кодовому слову
...
Информация об одном пользователе повещается в 163 байта, это значит, что все пользователи занимают 88744650 * 163 байт = 13 Гбайт. Это значит, что все пользователи поместятся на один шард.
