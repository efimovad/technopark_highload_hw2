...
В категории "личные вещи" обычно продаются вещи в единичном экземпляре, это значит, что  тогда нет необходимости хранить всю информацию о закрытом объявлении . Возможным решением проблемы является сохранения только основной информации после закрытия объявления (см. таблицу ниже).   
...
- в среднем для заключения сделки требуется 5-10 сообщений  
- объем одного сообщения 532 байта  
Из этих предположений среднее количество данных о сообщениях за год:   
120 сделок/мин * 60 * 24 *365 * 8 сообщений * 532 байта / (1024 * 1024 * 1024) = 250 Гбайт  
...
| микросервис  | rps_макс | память    | предел бд |
|--------------|----------|-----------|-----------|
| объявления   | 6000     | 169 Гбайт | 32 Тбайта |
| пользователи | 1000     | 16 Гбайт  | 64 Тбайта |
| сообщения    | 1000     | 250 Гбайт | 64 Тбайта |

Сервис с самой большой нагрузкой - это сервис объявлений, поэтому выберем машину с такой конфигурацией:  
| CPU(cores) | RAM(GB) | SSD(GB) |
|------------|---------|---------|
| 14         | 32      | 1024    |

Для сообщений достаточно такой конфигурации:  
| CPU(cores) | RAM(GB) | SSD(GB) |
|------------|---------|---------|
| 8          | 16      | 1024    |
...
В первую очередь Авито ориентирован на Россию, поэтому логично расположить сервера в России. При покупке облачных серверов нет необходимости в аппаратной поддержке. Будем использовать Яндекс Облако.
