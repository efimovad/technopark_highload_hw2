...
#### Объявления
? Таблица объявлений с основной информацией, позволит уменьшить размер бд для поиска списка объявлений
| id     | id пользователя | название    | id города | цена | дата      | аватар       | статус   |
|--------|-----------------|-------------|-----------|------|-----------|--------------|----------|
| bigint | bigint          | varchar(50) | bigint    | int  | timestamp | varchar(100) | smallint |
#####  Общие категории 
...
Для того, чтобы объединить все микросервисы будем использовать nginx. Благодаря своей архитектуре nginx способен поддерживать большое количество соединений и нужен для балансировки нагрузки особенно при большом количестве "медленных" соеднинений. 
